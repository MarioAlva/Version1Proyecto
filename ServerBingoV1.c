	...
	for (;;){
		...
		while(terminar==0)
		{
			...
			if(codigo==0) 	
				...
				char agregar[80];
				if(numero == 0){
					strcpy(agregar, "INSERT INTO Jugador (Username, Password, Nombre, Fondos) VALUES ('");					
			...
			else if (codigo==3){ //Fondos que tiene un jugador
				char nombre[20];
				printf ("Codigo: %d\n", codigo);
			...
			else if (codigo ==4){ //Nombres a los que se enfrenta un jugador
				...
				strcpy(consulta, "SELECT Game.Username FROM Game WHERE Game.Identificador = (SELECT Game.Identificador FROM Game WHERE Game.Username = Jugador.Username AND Jugador.Username = '");
				strcat(consulta, nombre);
				strcat(consulta, "');");
			...
			else if(codigo==5){ //Ranking de fondos
				printf ("Codigo: %d\n", codigo);
